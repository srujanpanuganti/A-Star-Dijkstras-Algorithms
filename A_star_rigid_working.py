...
## to display
def display_map(start_position,goal_position,node_path,visited_nodes,updated_circular_map,updated_map_point_robot):
...
def cost_to_go(current_node,goal_node):
    ...
    x1 = current_node[0][0][0]
    y1 = current_node[0][0][1]
...
#start_position = ([15,20],0)
#goal_position = [([140,100],0)]
grid_size = [150,250]
#grid_length = 5
    ...
    for action in explored_nodes:
        if is_visited_check(action,node_check_set) == False:
            if if_obstacle(action,circular_robot_obstacle_space_set) == False:
                ...
                #cost_to_goal = cost_to_go(action,goal_position)
