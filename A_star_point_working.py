...
def if_obstacle(node,obstacle_space_set):
    # print('node in the if obstacle',node[0][0])
    return str(node[0][0]) in obstacle_space_set

def if_obstacle_tuple(node,obstacle_space_set):
    # print('node in the if obstacle',node[0][0])
    return str(node) in obstacle_space_set


## to check if the node is visited
def is_visited_check(node, node_check_set):
    # print('node in is visited check',str(node[0][0]))
    return str(node[0][0]) in node_check_set

def is_visited_check_tuple(node, node_check_set):
    # print('node in is node check tuple',node)
...
def node_info_list():
    ...
    distance = {}
    for node in loc:
        distance[str(node)] = 999999 #math.inf
...
def cost_to_go(current_node,goal_node):
    ...
    # print('ndde in cost to go ',current_node)
...
def cost_to_go_tuple(current_node,goal_node):

    print('node in cost to go tuple',node)
    p1 = current_node
    p2 = goal_node[0][0]
#    p1 = [1,2]  #x1,y1
#    p2 = [5,6]  #x2,y2
...
while not q.empty() and is_a_vaid_input == True:
# while is_a_vaid_input == True or count == 10:
