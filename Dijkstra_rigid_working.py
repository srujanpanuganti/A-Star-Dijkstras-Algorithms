...
def read(start,radius,clear,grid,goal):
    start_pos = []
    goal_pos = []
    ...
    return s_pos, rad, cla, grid_sz, g_pos

#to get the input from the user
parser = argparse.ArgumentParser()
parser.add_argument('start_position')
parser.add_argument('robot_radius')
parser.add_argument('clearance')
parser.add_argument('grid_size')
parser.add_argument('goal_position')

args = parser.parse_args()
start_position, robot_radius,clearance, grid_length , goal_position = read(args.start_position,args.robot_radius,args.clearance,args.grid_size,args.goal_position)

#print(start_position, robot_radius,clearance, grid_length , goal_position)

print('start position = ',start_position[0])
print('goal position',goal_position[0][0])
...
## to define obstacle space at the boundary
def at_boundary_borders(clearance,circle_robot_radius):
	polygon_circular_vertices = np.array([[clearance+ circle_robot_radius,150 - (clearance+circle_robot_radius)],
										  [250 -(clearance+circle_robot_radius),150 -(clearance+circle_robot_radius)],
										  [250-(clearance+circle_robot_radius),150 - (150-(clearance+circle_robot_radius))],
										  [clearance+circle_robot_radius,150 - (150-(clearance+circle_robot_radius))]])
	new_map = np.ones([150,250])
	new_map_copy = np.zeros([150,250])
	cv2.fillPoly(new_map_copy,[polygon_circular_vertices],True,1)
	updated_map = new_map - new_map_copy
	#cv2.imshow('border map',updated_map)

	return updated_map

## to generate map with circular robot
def circular_robot_map(map_passed, ellipse_center, circle_bot_ellipse_axes, circle_center, circle_obstacle_radius,clearance,circle_robot_radius):

	#eli_center = tuple(list(correct_y_axis(ellipse_center)[0][0]))
	map = np.copy(map_passed)
	##### Half-planes defining the rectangle

	equation_1 = [0,1,-112.5-circle_robot_radius-clearance]   ### a1*x+b1*y+c1 = [a1,b1,c1]
	equation_2 = [1,0,-50+circle_robot_radius+clearance]
	equation_3 = [0,1,-67.5+circle_robot_radius+clearance]
	equation_4 = [1,0,-100-circle_robot_radius-clearance]
...
#	cv2.polylines(map,[polygon_circular_vertices],True,1)
#	cv2.fillPoly(map,[polygon_circular_vertices],True,1)
...
#	cv2.polylines(map,np.int32([polygon_vertices_circular_corrected]),True,(255,255,255))
	cv2.fillPoly(map,np.int32([polygon_vertices_circular_corrected]),True,1)
	...
	boundary_map = at_boundary_borders(clearance,circle_robot_radius)
	final_map = map + boundary_map
...
#### robot specification
circle_robot_radius = robot_radius  ## obtained from argpase
circle_robot_diameter = circle_robot_radius*2

#clearance = 3			 ## nee to read from the argparse

#print(ellipse_obstacle_axes[0],circle_robot_diameter,clearance)
### enlarged ellipse parameters
enlarged_ellipse_obstacle_axes = tuple(np.int32(((ellipse_obstacle_axes[0] / 2 + circle_robot_diameter / 2 + clearance), (ellipse_obstacle_axes[1] / 2 + circle_robot_diameter / 2 + clearance))))

#### enlarged circle parameters
enlarged_circle_obstacle_radius = np.int32((circle_robot_diameter + circle_obstacle_diameter)/2 + clearance)

##########  For circular robot  ##################
updated_circular_map = circular_robot_map(map, ellipse_center_corrected, enlarged_ellipse_obstacle_axes, circle_center_corrected, enlarged_circle_obstacle_radius,clearance,circle_robot_radius)
...
#if_visited(up_loc,node_check) == False:
...
## to display
def display_map(start_position,goal_position,node_path,visited_node,updated_circular_map,updated_map_point_robot):

	display_map = updated_map_point_robot

	plt.figure(figsize=(150,250))

	enlarged = updated_circular_map - updated_map_point_robot

	for i in range(0,enlarged.shape[0]):
		for j in range(0,enlarged.shape[1]):
			if enlarged[i,j] == 1:
				plt.plot(j,150-i,'y.',label = 'enlarged space due to radius and clearance')


	for i in range(0,display_map.shape[0]):
		for j in range(0,display_map.shape[1]):
			if display_map[i,j] == 1:
				plt.plot(j,150-i,'k.',label = 'obstacle space')

	for node in node_path:
		plt.plot(start_position[0][1],150-start_position[0][0],'bo',label = 'start postion')
		plt.plot(goal_position[0][0][1],150-goal_position[0][0][0],'go',label = 'goal position')
		plt.plot(node[1],150-node[0],'r+',label = 'path to goal')
...
## Generating the obstacle space set from the map
updated_circular_map = ceil_map(updated_circular_map)
#cv2.imshow('cr',updated_circular_map)
updated_map_point_robot = ceil_map(updated_map_point_robot)
#cv2.imshow('ps',updated_map_point_robot)
...
#start_position = ([11,11],0)
#goal_position = [([130,15],0)]
grid_size = [150,250]
#grid_length = 5

is_a_vaid_input = valid_start_goal(start_position,goal_position,circular_robot_obstacle_space_set)
#print(is_a_vaid_input)
...
while not q.empty() and is_a_vaid_input == True:# and :
    ...
    for action in explored_nodes:
        if is_visited_check(action,node_check_set) == False:
            if if_obstacle(action,circular_robot_obstacle_space_set) == False:
                ...
                node_info_parent_dict[str(action[0][0])] = node[0][0]   #--> parent is updated to the node info
...
if is_a_vaid_input == True:
    if goal_reached:
        ...
        display_map(start_position,goal_position,node_path,visited_node,updated_circular_map,updated_map_point_robot)
    else:
        print('goal cannot be reached')
else:
    print('enter a valid input: either your start position or goal position are in the obtacle space')
